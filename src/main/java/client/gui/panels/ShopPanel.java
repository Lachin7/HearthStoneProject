...
public class ShopPanel extends MyPanel implements ActionListener {
    ...
    public ShopPanel(ShopActionController actionController) {
        ...
        new MyButton("back to Menu", "pinkCrystal100.png", this, actionEvent -> actionController.back(),35, 620);
        new MyButton("exit", "pinkCrystal100.png", this, actionEvent -> actionController.exit(),150, 620);
    }

    public void setBuyShowCase(HashMap<MyPair<Long, String>, Integer> lockedCards, long coins) {
        cardsShowCase.removeAll();
        System.out.println("set buyyy");
        playersCoins.setText("you have " + coins + " coins");
        for (Map.Entry<MyPair<Long, String>, Integer> card : lockedCards.entrySet()) {
            new MyCardButton(actionController, card.getKey().getKey(), card.getKey().getValue(), 100, cardsShowCase, actionEvent -> {
                cardPanel.remove(selectedCard);
        ...
        cardsShowCase.revalidate();
    }

    public void setSellShowCase(HashMap<MyPair<Long, String>, Integer> unLockedCards, long coins) {
        cardsShowCase.removeAll();
        playersCoins.setText("you have " + coins + " coins");
        for (Map.Entry<MyPair<Long, String>, Integer> card : unLockedCards.entrySet()) {
            new MyCardButton(actionController, card.getKey().getKey(), card.getKey().getValue(), 100, cardsShowCase, actionEvent -> {
                cardPanel.remove(selectedCard);
                selectedCard = new MyCardButton(actionController, card.getKey().getKey(), card.getKey().getValue(), 200, cardPanel, null, false);
                price.setText("    price        :      " + card.getValue());
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if (button == CardsYouCanBuyButton) {
            ...
            if (selectedCard==null || selectedCard.getName().equals(""))JOptionPane.showMessageDialog(null,"you should select a card first ");
            else{
                cardPanel.remove(selectedCard);
