...
public class CollectionPanel extends MyPanel implements ChangeListener{
    ...
    private void makeHorizontalBar() {
        ...
        unlockedCards = new MyButton("Unlocked Cards", "blueCrystal120.png", horizontalBar, actionEvent -> actionController.setShowCase(-1, null, null, null, false, false));
...
//                    actionController.getClient().getShopPanel().getSelectedCard().setOpaque(false);
                    actionController.getClient().getShopPanel().getSelectedCard().setName(entry.getValue());
                    actionController.log("button clicked to buy a card - Collection - changing panel to shop");
                    actionController.getClient().goToPanel(actionController.getClient().getShopPanel());
                }
                if (ans == 1) {
                    if (currentDeck == null)
                        JOptionPane.showMessageDialog(cardsShowCase, "first choose a deck from the deck bar ");
                    ...
                    if (currentDeck != null) actionController.updateDeckShowCase(currentDeck);
                }
            }, false);
    ...
    public void updateDeckCards(String hero, HashMap<Long,String> cards) {
        ...
        for (Map.Entry<Long,String> entry : cards.entrySet()) {
            new MyCardButton(actionController, entry.getKey(), entry.getValue(), 100, cardsShowCase, actionEvent -> {
